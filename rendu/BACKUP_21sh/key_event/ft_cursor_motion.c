...
/*   Updated: 2017/03/31 15:53:57 by gubourge         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_cursor_more_right(t_term *term, int len_line)
{
	...
	tmp_pos = (term->pos_x + 1) + ft_strlen(term->prompt) - (term->pos_y * argp.ws_col);
...
void		ft_cursor_more_left(t_term *term)
{
	...
	tmp_pos = (term->pos_x + 1) + ft_strlen(term->prompt) - (term->pos_y * argp.ws_col);
	if (term->pos_y > 0)
		tmp_pos += (int)ft_strlen(term->prompt);
	while (tmp_pos > (1 + (int)ft_strlen(term->prompt)))
	{
		term->res = tgetstr("le", &(term->area));
		tputs(term->res, 1, &ft_outc);
		(term->pos_x)--;
