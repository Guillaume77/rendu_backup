...
/*   Updated: 2017/03/29 18:36:48 by gubourge         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		init_tab_func(t_func_ptr *tab_func)
{
	tab_func[0] = &ft_echo;
	tab_func[1] = &ft_cd;
	tab_func[2] = &ft_setenv;
	tab_func[3] = &ft_unsetenv;
	tab_func[4] = &ft_env;
	tab_func[5] = &ft_exit;
	tab_func[6] = NULL;
}
...
int			init_builtin(t_builtin *builtin)
{
	if (!(builtin->tab_built = ft_strsplit(BUILTIN, ':')))
		return (-1);
	init_tab_func(builtin->tab_func);
